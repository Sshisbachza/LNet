...
namespace LNetServer
{
    public class TCPServer : IServer
    {
        ...
        //| Using custom client                                              |
        //+------------------------------------------------------------------+
        private List<CustomClient> usersListCustom = new List<CustomClient>();
        private bool customClient = false;
        private bool Logging = false;

        public TCPServer() { }

        public TCPServer(string hostname, int port, ServerOptions serverOptions)
        {
            ipAddressValue = hostname;
            portNumber = port;
            tcpListener = new TcpListener(IPAddress.Parse(hostname), port);
            listenThread = new Thread(new ThreadStart(ListenMethodCustom));

            customClient = serverOptions.bIsCustomClient;
            Logging = serverOptions.bLogging;
        }
        ...
        public void Stop()
        {
            SendMessageToAll(new AMessage(MessageType.LEAVE, -1, "Servidor desconectado"));
        ...
        private void ListenMethodCustom()
        {
            isListening = true;
            userList.Clear();
            ...
            try
            {
                while (isListening)
                {
                    ...
                    CustomClient client = new CustomClient(tcp, stream, clientNumber);

                    binary.Serialize(stream, new AMessage(MessageType.JOIN, clientNumber, "Cliente Conectado com Sucesso"));
                    messageQueue.Enqueue("Cliente " + client.ClientNumber + " Conectado");

                    AMessage joinedMessage = binary.Deserialize(stream) as AMessage;
                    foreach (CustomClient user in usersListCustom)
                    {
                        binary.Serialize(user.Stream, joinedMessage);
                        user.Stream.Flush();
                    }
                    usersListCustom.Add(client);

                    client.Thread = new Thread(new ParameterizedThreadStart(ClientMethodCustom));
        ...
        private void ClientMethodCustom(Object obj)
        {
            CustomClient client = obj as CustomClient;
